"""Generate Hybrid Query (HQ1 and HQ2) bar charts for records vs latency."""
...
LEGEND_NAMES: Mapping[str, str] = {
    "APEX-2bit": "APEX (2-bit)",
    "APEX-4bit": "APEX (4-bit)",
    "APEX-8bit": "APEX (8-bit)",
}
SERIES_ORDER = [
    "APEX-8bit",
    "APEX-4bit",
...
# X-axis label mapping for records
RECORDS_LABELS = {
    4096: r"$2^{12}$",
    16384: r"$2^{14}$",
    65536: r"$2^{16}$",
...
def load_dataframe(csv_path: Path, dataset_label: str) -> PlotData | None:
    ...
    tag = f"hybrid_query[{dataset_label}]"
...
def plot_bars(data: PlotData, output_path: Path, dataset_label: str) -> None:
    ...
    with plt.rc_context(STYLE_OVERRIDES):
        ...
        if not valid_series:
            print(
                f"[hybrid_query[{dataset_label}]] No valid data to plot.",
                file=sys.stderr,
            )
            plt.close(fig)
            return
        ...
        for x_idx in range(len(x_values)):
            ...
            # Both HQ1 and HQ2: baseline is 8-bit (slowest)
            baseline_val = max(group_values)

            # Annotate each bar
            for val, pos, series in zip(group_values, group_positions, group_series):
                # Speedup relative to 8-bit (slowest): 8-bit = 1×, faster configs > 1×
                multiplier = baseline_val / val

                # HQ2: integer format, HQ1: decimal format
                if dataset_label == "hq2":
                    label_text = f"{multiplier:.0f}×"
                else:
                    label_text = f"{multiplier:.1f}×"

                # Position label above bar
                _, _, bars = bars_by_series[series]
                bar_height = val

                # Adjust positioning - HQ1 lower, HQ2 normal
                x_offset = 0
                if dataset_label == "hq1":
                    y_scale = 1.02  # Lower position for HQ1
                else:
                    y_scale = 1.08  # Normal position for HQ2
        ...
        # Format x-axis to show record counts as powers of 2
        x_labels = [RECORDS_LABELS.get(int(x), str(int(x))) for x in x_values]
    ...
    print(f"[hybrid_query[{dataset_label}]] Wrote figure to {pretty_path}")


def main() -> None:
    # Generate both HQ1 and HQ2 plots
    for query_name in ["hq1", "hq2"]:
        csv_path = DATA_DIR / f"{query_name}.csv"
        output_path = OUTPUT_DIR / f"{query_name}.pdf"

        data = load_dataframe(csv_path, query_name)
        if not data:
            sys.exit(1)

        plot_bars(data, output_path, query_name)
